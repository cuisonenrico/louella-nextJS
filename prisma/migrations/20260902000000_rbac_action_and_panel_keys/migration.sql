-- Register the action and panel keys from src/lib/rbac/features.ts.
--
...
--   saved override. These keys are a schema-level prerequisite for the
--   component-level permission tier, not optional sample data.
--
-- WHAT CHANGED
--   The matrix used to stop at the screen: 'products' was the smallest thing an
--   admin could grant or revoke. It now carries two child tiers, flattened to
--   '<feature>:<child>' and stored here as ordinary keys:
--     actions - a write verb a screen offers   ('products:delete')
--     panels  - a region of a screen           ('dashboard:revenue-trend')
--   Plus one cross-cutting key, 'all-branches', which drives BranchGuard.
--
-- WHAT IT DOES NOT DO
...
--   silently diverge the database from the defaults in code. In particular the
--   narrowed MANAGER default is a code change, not a data change, so it can be
--   reviewed and reverted like one.
--
-- BACKWARD COMPATIBILITY
--   Purely additive - every pre-existing key keeps its row and its label. The
--   frozen Cloud Run image and the shipped Flutter build read only the original
--   ten keys and ignore rows they do not recognise.
