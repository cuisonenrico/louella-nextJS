...
export class InventoryImportService {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      for (const [productId, counts] of accumulator) {
        await tx.inventory.upsert({
          ...
          // The sheet is the source of truth for the day's counts. Its opening
          // ("Yesterday LO") is written as given, then carry-forward below
          // replaces it with the previous day's close wherever one exists —
          // the rule every other writer follows. Within one workbook those
          // agree; where they don't, the stored previous day wins.
          // isAutoGenerated flips so carry-forward never treats an imported
          // row as a placeholder whose close it may recompute.
          //
          // deletedAt is cleared for the same reason InventoryService.create
          // clears it: the unique key excludes deletedAt, so a soft-deleted row
          // still owns the slot and would swallow the import silently.
          update: {
            ...counts,
  ...
  async dryRunWorkbook(
    ...
    if (branchId !== undefined) {
      // A preview reveals which of a branch's days already hold data, so it is
      // scoped on the same boundary as the import itself.
  ...
  async importWorkbook(
    ...
    // BranchGuard cannot cover this endpoint — branchId arrives in a multipart
    // body, which multer only populates *after* guards have run — so an import
    // would otherwise let a branch-scoped manager overwrite any other branch's
    // month. See common/utils/branch-access.ts.
    assertBranchAccess(user, branchId);
