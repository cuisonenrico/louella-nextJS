...
describe('InventoryImportService', () => {
  ...
  describe('importWorkbook', () => {
    ...
    it('upserts matched products and records unmatched names as row errors', async () => {
      ...
      const res = await service.importWorkbook(
        ...
        7,
        'sheet.xlsx',
        { id: 55, branchId: null, permissions: ['all-branches'] },
        'skip',
        [],
        ['Unknown Item'],
      );
    ...
    it('clears deletedAt so importing over a deleted day restores the row', async () => {
      ...
      // Without clearing the tombstone the import reports success and writes
      // rows that no read can see — every read filters `deletedAt: null`.
      const buf = buildWorkbook([
        {
          name: 'Day (1)',
          dateHeader: '4/14/26',
  ...
  // Branch isolation
  //
  // The controller cannot delegate this to BranchGuard: branchId arrives in a
  // multipart body, and multer is an interceptor, which Nest runs *after*
  // guards — so req.body is still empty when the guard looks. The check has to
  ...
  describe('branch isolation', () => {
    ...
    const unscoped = () => ({
      id: 1,
      branchId: null,
      permissions: ['inventory-import', 'all-branches'],
    });
    ...
    it('refuses an import into a branch the user is not scoped to', async () => {
      await expect(
        service.importWorkbook(workbook(), 7, 'sheet.xlsx', scoped(2)),
      ).rejects.toThrow(ForbiddenException);
      expect(prisma.inventory.upsert).not.toHaveBeenCalled();
    });

    it('allows an import into the branch the user is scoped to', async () => {
      const res = await service.importWorkbook(
        workbook(),
        7,
        'sheet.xlsx',
    ...
    it('allows an all-branches user to import into any branch', async () => {
      const res = await service.importWorkbook(
        workbook(),
        7,
        'sheet.xlsx',
    ...
    it('refuses a dry run against another branch', async () => {
      await expect(
        service.dryRunWorkbook(workbook(), 'sheet.xlsx', 7, scoped(2)),
      ).rejects.toThrow(ForbiddenException);
    });

    it('confines a scoped log listing to the assigned branch', async () => {
    ...
    it('refuses a scoped listing of another branch logs', async () => {
      await expect(
        service.getLogs({ branchId: 7, page: 1, limit: 20 }, scoped(2)),
      ).rejects.toThrow(ForbiddenException);
    });
  });
});
