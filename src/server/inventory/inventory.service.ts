...
export class InventoryService {
   ...
   *
   * On a placeholder — a day nobody has counted — the closing balance rises by
   * the delivery too. A placeholder closes on everything it has (see
   * buildCarryForward); raising only `delivery` made computeSold() report the
   * whole delivery as sold before anyone had counted a single piece. The
   * following placeholder days are then carried forward from the new close.
   *
   * A missing row is created as a placeholder that carries forward the
   * previous day's leftover, like every other placeholder (it used to open at
   * 0, which dropped the stock on hand).
   */
  async addDeliveryInTx(
    ...
    const row = await tx.inventory.findUnique({
      ...
      select: { id: true, isAutoGenerated: true, deletedAt: true },
    });

    let closing: number;
    if (!row) {
      const prior = await tx.inventory.findFirst({
        where: { branchId, productId, date: { lt: date }, deletedAt: null },
        orderBy: { date: 'desc' },
        select: { leftover: true },
      });
      const opening = prior?.leftover ?? 0;
      ...
      const updated = await tx.inventory.update({
        ...
        data: {
          delivery: { increment: quantity },
          leftover: { increment: quantity },
          deletedAt: null,
        },
        select: { leftover: true },
      });
      closing = updated.leftover;
    } else {
      // A counted row: its leftover is a physical count and stays as typed.
      await tx.inventory.update({
        where: { id: row.id },
        data: { delivery: { increment: quantity }, deletedAt: null },
      });
      return;
    }

    const subsequent = await tx.inventory.findMany({
      where: { branchId, productId, date: { gt: date }, deletedAt: null },
      orderBy: { date: 'asc' },
      select: {
        id: true,
        isAutoGenerated: true,
    ...
    for (const w of this.buildCarryForward(closing, subsequent)) {
      await tx.inventory.update({
        where: { id: w.id },
        data: { quantity: w.quantity, leftover: w.leftover },
      });
    }
  }
